...
import { getGateById } from '@/features/demo/lib/gates';
import { buildTasteGatePrompt, CLAUDE_MODEL } from '@/features/demo/lib/taste-gate-prompt';
...
export async function GET(request: Request) {
  ...
  if (!gateId) {
    return jsonError(400, 'Missing gate parameter');
  }

  const gate = getGateById(gateId);
  if (!gate) {
    return jsonError(400, `Unknown gate: ${gateId}`);
  }
  ...
  try {
    ...
    const prompt = buildTasteGatePrompt(canon, evolutions, gate);
